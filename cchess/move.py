...
class Move(object):
    ...
    def chinese_move_to_std_move(man_kind, move_side, p_from, move_str):
        ...
        if man_kind in ['k', 'r', 'c', 'p']:
            ...
            if move_str[0] == "平":
                ...
                new_x = get_h_level(move_side,move_str[1])
                return (new_x, p_from[1])
            else:
                #王，车，炮，兵的前进和后退
                #diff = v_change_index[move_side].index(move_str[1])
                diff = get_v_change(move_side,move_str[1])
                if move_str[0] == "退":
                    diff = -diff
    ...
    def from_chinese(board, move_str):
        ...
        if not fench:
            try:
                x = get_h_level(board.move_side,man_name)
                fench= chinese_to_fench('兵', board.move_side)
            except:
                return None
        ...
        if not multi_mans:
            ...
            x = get_h_level(move_side,move_str[1])
            ...
            if man_kind=='p' and len(board.get_fenchs(fench))>2:
                #多兵选一(大于二)
                if x in locals().keys():
                    #第二个字是纵线序号
                    poss = board.get_fenchs_x(x, fench)
                    move_indexs = {"前": 0,"中":len(poss)//2, "后": -1,"一":0, "二":1, "三":2, "四":3, "五":4}
                    pos = poss[move_indexs[move_str[0]]]
                else:
                    #第二个字是兵或卒
                    allposs=[]
                    for i in range(9):
                        poss = board.get_fenchs_x(i, fench)
                        allposs.append(len(poss))
                    allposs_=allposs.copy().sort(reverse=True)
                    if allposs_[0]%2:
                        #最多列有奇数个
                        move_indexs = {"前": 0,"中":allposs_[0]//2, "后": -1,"一":0, "二":1, "三":2, "四":3, "五":4}
                        index=move_indexs[move_str[0]]
                        if index<=1 and move_str[0]!='中' and allposs_[1]==2:
                            return None
                    else:
                        #最多列有偶数个
                        if allposs_[1]==2:
                            return None
                        move_indexs = {"前": 0, "后": -1,"一":0, "二":1, "三":2, "四":3}
                        index=move_indexs[move_str[0]]
                    x=allposs.index(allposs_[0])
                    pos = board.get_fenchs_x(x, fench)[index]
                move = Move.chinese_move_to_std_move(man_kind, move_side, pos,
                                                     move_str[2:])
            
            elif move_str[0] in ['前', '后']:
                #二子选一      
