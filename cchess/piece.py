...
v_change_index = ((), ('错', '一', '二', '三', '四', '五', '六', '七',
                       '八', '九'), ('误', '１', '２', '３', '４', '５', '６', '７', '８', '９'))
...
def get_h_level(side,nstr):
    ...
    if i >=0:
        if side==ChessSide.BLACK:
            ...
            return 8-i
    raise ValueError(nstr,"not in tuple indexes [get_h_level]")

def get_v_change(side,nstr):
    i=-1
    for t in indexes:
        try:
            i=t.index(nstr)
    ...
    if i >=0:
        return i+1
    raise ValueError(nstr,"not in tuple indexes [get_v_change]")
#-----------------------------------------------------#
