...
class BaseChessBoard(object):
    ...
    def dump_board_compact(self,board_type=0):
        board_str = _text_board_compact[board_type][:]
        y=10
        for line in self._board:
            x=0
            for ch in line[:]:
                if ch:
                    new_text = board_str[y][:x] + fench_to_txt_name(ch) + board_str[y][x + 1:]
                    board_str[y] = new_text
                x+=1
            y-=1
        return board_str
#-----------------------------------------------------#
...
class ChessBoard(BaseChessBoard):
    ...
    def is_dead(self):
        if not self.get_king(self.move_side):
            return True
        for piece in self.get_pieces(self.move_side):
